...
export function getWikidataEditObject(
  ...
  // );
  const WD_YOUTUBE_VIDEO = "Q63412991";
  ...
  //youtube
  const WD_STANDARD_YOUTUBE_LICENSE = "Q111772254";
  const WD_CC_BY_30 = "Q149475462";
  const WD_YOUTUBE_MADE_FOR_KIDS = "Q111772463";

  let license = null;
  if (license === "youtube") {
    license = WD_STANDARD_YOUTUBE_LICENSE;
    ...
    claims[WD_RECORDING_DATE] = [
      {
        ...
        references: {
          [WD_BASED_ON_HEURISTIC]: WD_INFERRED_FROM_YOUTUBE_DESCRIPTION,
  ...
  if (video.status.madeForKids) {
    hasQuality.push(WD_YOUTUBE_MADE_FOR_KIDS);
  }
