...
export async function getEpisodesById(podcast: string, limit: number) {
  const ORDER_BY = "DESC";
  let query = `#podcast
SELECT ?item ?itemLabel ?title ?url ?publicationDate ?duration ?hasQuality ?seasonNumber ?episodeNumber ?recordedAtLabel ?productionCode (GROUP_CONCAT(DISTINCT ?guestLabel;separator=", ") AS ?guests) (GROUP_CONCAT(DISTINCT ?mainSubjectLabel;separator=", ") AS ?topics) 
WHERE 
{
...
GROUP BY ?item ?itemLabel ?title ?url ?publicationDate ?duration ?hasQuality ?seasonNumber ?episodeNumber ?recordedAtLabel ?productionCode
